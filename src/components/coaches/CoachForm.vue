...
<form @submit.prevent="submitForm">
    ...
    <div class="form-control" :class="{invalid: !description.isValid}">
        ...
        <textarea type="text" id="description" v-model.trim="description.val" rows="5"  @blur="clearValidity('description')"></textarea>
        <p v-if="!description.isValid">Description not be empty</p>
    </div>
    <div class="form-control" :class="{invalid: !rate.isValid}">
        <label for="rate">Hourly Rate</label>
        <input type="number" id="rate" v-model.number="rate.val">
        <p v-if="!lastName.isValid">Rate must be greater than 0</p>
    </div>
    <div class="form-control" :class="{invalid: !areas.isValid}">
      ...
      <base-button>Resgister</base-button>
</form>
</template>
